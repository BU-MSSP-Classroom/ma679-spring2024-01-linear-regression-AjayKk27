...
So we'll start by doing a little bit of eda to see what variables make a lot of sense. 
```{r}
#One thing I looked at while doing EDA, because the data coincides with the 2008 financial crisis was why housing prices didn't seem to change much over the years at all. What did change was the number of houses sold. Potentially the number of houses sold would be a good variable to include. 

ames_raw_2008<- ames_raw_2008 |> group_by(`Yr Sold`) |> mutate(
                                      number_sold = n()
)

#So is number of households a good number? Let's see how that works with house price. 

ames_raw_2008_numHouses<-ames_raw_2008
ames_raw_2008_numHouses$number_sold <- factor(ames_raw_2008_numHouses$number_sold)

ggplot(data=ames_raw_2008_numHouses, aes(number_sold, SalePrice))+
  geom_boxplot()

 


#Moving on to housing quality
ames_raw_2008_1$`Overall Qual`<- factor(ames_raw_2008_1$`Overall Qual`)
ames_raw_2008_1$`Overall Cond`<- factor(ames_raw_2008_1$`Overall Cond`)



#Further, the sale price of the house seems to change in a pronounced manner with the overall quality. However this relationship is not linear but we can put a suitably transformed quality variable into the regression (e.g quality^2). But what is a suitable transformation?

eq = function(x){(x*x)*10000}
eq2= function(x){(x^1.5)*10000}
eq3= function(x){(x^1.7)*10000}
ggplot(data=ames_raw_2008_1, aes(`Overall Qual`, SalePrice))+
 geom_boxplot()+
  geom_function(data=ames_raw_2008, aes(x=`Overall Qual`), fun = eq, color = 'red')+
  geom_function(data=ames_raw_2008, aes(x=`Overall Qual`), fun = eq2, color = 'blue')+
  geom_function(data=ames_raw_2008, aes(x=`Overall Qual`), fun = eq3, color = 'orange')

#The question here is which one to pick. x^1.7 might appear to be the best but we might also be guilty of overfitting if we use that. Similarly x^2 seems to overestimate the effect of quality. 
  
#So what other variables might be appropriate?

#Overall Condition
ggplot(data=ames_raw_2008_1, aes(`Overall Cond`, SalePrice))+
 geom_boxplot()

#Not so good, we can see that there are loads of outliers for each condition, especially at the higher levels. 

ggplot(data=ames_raw_2008, aes(Neighborhood, SalePrice))+geom_boxplot()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

#What about combining house square footage? This looks like it'll capture at least some of what makes some houses more expensive than others. 
ggplot(data=ames_raw_2008,aes(`1st Flr SF`+`2nd Flr SF`, SalePrice))+geom_line()

#This seems like a decent level of complexity. 

ames_raw_2008_2<- ames_raw_2008_2 |> mutate(
                                        qual =`Overall Qual`^1.7,
                                        number_houses_sold = n()
                                        )

fit1<- lm(SalePrice~qual+Neighborhood+`Gr Liv Area`, data=ames_raw_2008_2)
summary(fit1)

#Can I improve this model? W

```


```{r,eval=FALSE,echo=show_code}
ames_raw_2008<- ames_raw_2008 |> mutate(
                                        qual =`Overall Qual`^1.7
                                        )

lmfit_2008<- lm(SalePrice~qual+Neighborhood+`Gr Liv Area`, data=ames_raw_2008)# ["your model here"] # use #ames_raw_2008
```

When you decide on your model use the following to come up with your test loss.
```{r,eval=FALSE,echo=show_code}
#Adding my square feet and qual = (Overall Qual) variables to ames_raw_2009:
ames_raw_2009<- ames_raw_2009 |> mutate(
                                        qual =`Overall Qual`^1.7)
...
This is okay but might a log scaled SalePrice model do better? Let's see:
```{r,eval=FALSE,echo=show_code}

lmfit_2008<- lm(log(SalePrice)~`Overall Qual` +Neighborhood+`Gr Liv Area`, data=ames_raw_2008)# ["your model here"] # use #ames_raw_2008
```
...
The model R2's indicate a decent fit. The large number of outliers in sales price probably affect the fit. 
...
predictedmpg<- 39.935861-0.157845*98
```

Your answer:

i. So there does seem to be some relationship between the predictor and response, given that the p values for  are so small. 
ii. The relationship is strong because the p values that our coefficients are indistinguishable from zero are very small.  
iii. The response and predictor have a negative relationship. 
iv. 24.476mpg.  
...
We have two issues here - first that the condition of homoscedasticity seems to be violated. Further we can see from the fitted vs residuals plot that the relationship between mpg and horsepower isn't really linear. 
~~~
Please write your answer in full sentences.
...
#Note this seems to be easier to see when the above command is pasted into the console as opposed to run here. 
```
...
auto_without_name <- auto |> select(-name)

cor(auto_without_name)
```
...
i & ii.. Yes there seems to be a relationship between the predictors and response. Especially in the case of displacement, weight, year and origin because a CI constructed around their estimates does not contain zero.

iii. The coefficient for the year variable suggests that on average, a car that was built a year after another one will on average have a0.75mpg higher mileage .
~~~
Please write your answer in full sentences.
...
We cans ee that there are in fact three points that are large outliers. These are the ones labeled 323, 325 and 327.
The leverage plot does not identify any points as having outsize influence(high leverage) as all points are within the cook distance boundary. 
...
```{r,echo=TRUE}
#
#I'll use an interaction of displacement with horsepower. For the same displacement, we can see differing horsepowers and potentially we'll see how that affects mileage. 

fit3<- lm(mpg~displacement+horsepower+weight+ horsepower*displacement, data=auto)
summary(fit3)
```

Your answer:

We can see immediately that the displacement:horsepower is statistically significant. 
...
```{r}
fit4<- lm(mpg~cylinders^2+displacement+horsepower+weight+acceleration+log(year)+origin, data=auto)
summary(fit4)
```
